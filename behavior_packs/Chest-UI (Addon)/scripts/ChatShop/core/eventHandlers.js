...
class EventHandlers {
    onShopInteract = (event) => {
        try {
            ...
            if (!shop) {
                const equip = player.getComponent('minecraft:inventory')?.container?.getSlot(player.selectedSlotIndex);
            ...
            const expectedHash = helpers.blockKey(block.location.x, block.location.y, block.location.z);
            if (shop.container.containerHash !== expectedHash) {
            ...
            if (shop.owner.playerId === player.id) {
                system.run(() => {
                    if (player.isValid) manageItems.manageItems(player, shop);
                });
            } else {
                ...
                system.run(() => {
                    if (player.isValid) buyMenu.showBuyMenu(player, shop);
                });
            }
        } catch (error) {
    ...
    onShopBreak = (event) => {
        try {
            ...
            const shop = data.shops[protectedEntry.shopId];
            if (!shop) return;
