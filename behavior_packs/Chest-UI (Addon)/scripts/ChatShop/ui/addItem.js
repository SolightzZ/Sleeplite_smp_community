...
class AddItem {
    addItem = (player, shop) => {
        try {
            ...
            const freeSlot = Array.from({ length: container.size }).findIndex((_, i) => !container.getItem(i));
            ...
            this.showAddForm(player, shop, container, freeSlot);
        } catch (error) {
    ...
    showAddForm = (player, shop, chestContainer, slotIndex) => {
        try {
            ...
            chest.title('เลือกสินค้าจาก Inventory');
            chest.pattern(['xxxxxxxxx', '_________', '_________', '_________', 'xxxxxxxxx', '_________'], {
                x: {
                    itemName: '',
            ...
            Array.from({ length: 27 }, (_, i) => i + 9).forEach((i) => {
                ...
                chest.button(i, `§f${helpers.formatName(item.typeId)}`, [`§7x${item.amount}`, '', '§eคลิกเลือก'], item.typeId, Math.min(item.amount, 99));
                slotMap[i] = i;
            });
            ...
            Array.from({ length: 9 }).forEach((_, i) => {
                ...
                chest.button(chestSlot, `§f${helpers.formatName(item.typeId)}`, [`§7x${item.amount}`, '', '§eคลิกเลือก'], item.typeId, Math.min(item.amount, 99));
                slotMap[chestSlot] = i;
            });
            ...
            uiUtils.showForm(player, chest, 'shop.add.pick', (res) => {
                ...
                uiUtils.showForm(player, form, 'shop.add.form', (res) => {
                    ...
                    if (!currentItem || currentItem.typeId !== inputItemId || currentItem.amount < amount) {
                    ...
                    if (itemStack.amount > amount) {
                        ...
                        player.sendMessage(`§e[Shop] ขาย ${amount} ชิ้น (NBT ถูกเก็บไว้) ส่วนเกิน ${extraAmount} ชิ้นถูกส่งคืน`);
                    } else {
                        chestContainer.setItem(slotIndex, itemStack);
                    }
                    ...
                    player.sendMessage(`§a[Shop] เพิ่มสินค้าสำเร็จ! (${amount} ${helpers.formatName(inputItemId)} ราคา ${price} $)`);
                });
            });
        } catch (error) {
