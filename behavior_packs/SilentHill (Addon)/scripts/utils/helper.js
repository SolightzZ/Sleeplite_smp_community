...
export function decrementStack(player) {
  ...
  if (item.amount == 1) inventory.setItem(player.selectedSlotIndex, undefined);
  else inventory.setItem(player.selectedSlotIndex, new ItemStack(item.typeId, item.amount - 1));
}
...
export function doesBlockBlockMovement(block) {
  return block.typeId != "minecraft:cobweb" && block.typeId != "minecraft:bamboo_sapling" && !block.isLiquid && !block.isAir;
}
...
export function shouldConnect(block) {
  ...
  if (id.includes("fence") || id.includes("wall") || id.includes("gate") || id.includes("pane") || id.includes("iron_bars")) {
    return true;
  }
