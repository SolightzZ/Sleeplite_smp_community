...
world.beforeEvents.chatSend.subscribe((e) => {
  if (
    e.message.startsWith(bridge.bedrockCommands.prefix) ||
    settings.ignore_prefixes.includes(e.message.charAt(0))
  )
    return;
  ...
  if (settings.change_nametags && settings.use_nametags) {
    ...
    world.sendMessage(
      t1 +
        (
          e.sender.getTags().filter((t) => t.startsWith("rank:"))[0] ??
          settings.default_rank
        ).slice(5) +
        t2 +
        (settings.use_nametags ? e.sender.nameTag : e.sender.name) +
        t3 +
        e.message +
        t4,
    );
...
if (settings.change_nametags) {
  for (const player of world.getAllPlayers()) {
    player.nameTag =
      t1 +
      (
        player.getTags().filter((t) => t.startsWith("rank:"))[0] ??
        settings.default_rank
      ).slice(5) +
      t2 +
      player.name;
  ...
  world.afterEvents.playerSpawn.subscribe((e) => {
    e.player.nameTag =
      t1 +
      (
        e.player.getTags().filter((t) => t.startsWith("rank:"))[0] ??
        settings.default_rank
      ).slice(5) +
      t2 +
      e.player.name;
...
bridge.events.chatDownStream.subscribe((e) => {
  world.sendMessage(
    t1 +
      "§9" +
      (e.roles[0] ?? settings.default_rank.slice(5)) +
      "§r" +
      t2 +
      e.author +
      t3 +
...
bridge.events.chatUpStream.subscribe((e, player) => {
  if (settings.change_nametags && settings.use_nametags) {
    ...
    e.author = (
      ...
      t2 +
      (settings.use_nametags ? player.nameTag : player.name)
    ).toDiscord();
  }
});
    ...
    if (target && role in settings.roles) {
      system.run(() => {
        target.getTags().forEach((t) => {
          if (t.startsWith("rank:")) target.removeTag(t);
        });
      ...
      player.sendMessage(
        "§cWrong command usage. Usage: addTag <username> <tag>\n§r§oNote: tag must be a registered tag",
      );
    }
  },
