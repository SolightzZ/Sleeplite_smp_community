...
const scoreboard_name = "esploratori:overall_time",
  session_scoreboard_name = "esploratori:session_time";
const timeScoreboard =
  world.scoreboard.getObjective(scoreboard_name) ??
  world.scoreboard.addObjective(scoreboard_name, "total play time (min)");
const sessionScoreboard =
  world.scoreboard.getObjective(session_scoreboard_name) ??
  world.scoreboard.addObjective(session_scoreboard_name, "session time (min)");

const joinTime = new Map();
const scoreboardIds = new Map();

world.getAllPlayers().forEach(async (player) => {
  //this line is here in case someone runs /reload which will erase stored data for online players
  if (!player) return;
...
world.afterEvents.playerLeave.subscribe((e) => {
  const session_time = Math.floor(
    1.667e-5 * (Date.now() - joinTime.get(e.playerId)),
  );

  if (scoreboardIds.has(e.playerId)) {
    // ignore gake players, who haven't been added to the map
    timeScoreboard.setScore(
      scoreboardIds.get(e.playerId),
      timeScoreboard.getScore(scoreboardIds.get(e.playerId)) + session_time,
    );
...
bridge.bedrockCommands.registerCommand(
  ...
  (player) => {
    var time =
      timeScoreboard.getScore(player.scoreboardIdentity) +
      sessionScoreboard.getScore(player.scoreboardIdentity);
...
system.runInterval(() => {
  world.getAllPlayers().forEach((p) => {
    if (!p) return;
    const session_time = Math.floor(
      1.667e-5 * (Date.now() - joinTime.get(p.id)),
    );
    p.runCommand(
      `scoreboard players set @s ${session_scoreboard_name} ${session_time}`,
    );
  });
}, 1200);
