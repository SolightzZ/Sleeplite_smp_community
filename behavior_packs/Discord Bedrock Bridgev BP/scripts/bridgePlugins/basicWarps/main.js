  ...
  async (user) => {
    if (player_cooldown.has(user.id)) {
      if (Date.now() - player_cooldown.get(user.id) < options.cooldown * 1000) {
        user.sendMessage(
          "§cYou cannot run this command now. Command in cooldown...",
        );
        return;
      } else {
    ...
    const message = await new Promise((acc) => {
      system.run(() => {
        ...
        if (spawn) {
          ...
          const location = dimensions.overworld.getBlockFromRay(
            { x: x, y: 400, z: z },
            { x: 0, y: 0, z: -1 },
            { maxDistance: 500 },
          )?.block.location;
...
bridge.bedrockCommands.registerCommand(
  ...
  (user, warp_name) => {
    ...
    if (warp_name in warps) {
      ...
      user.sendMessage(
        "§cUnknown warp name, please check available warps running §olistWarp",
      );
    }
  },
