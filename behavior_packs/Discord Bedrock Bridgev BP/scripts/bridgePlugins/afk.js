...
const afk_scoreboard =
  world.scoreboard.getObjective("esploratori:afk-time") ??
  world.scoreboard.addObjective("esploratori:afk-time", "afk-time");

function distance(v1, v2) {
  // Vector mock as @minecraft/math requires manifest change
  return Math.sqrt(
...
class PlayerStats {
  ...
  equals(stat) {
    return (
      distance(this.location, stat.location) < PlayerStats.error &&
...
bridge.bedrockCommands.registerCommand(
  ...
  (caller) => {
    if (afk_scoreboard.hasParticipant(caller)) {
      caller.sendMessage(
        `§oYou have been afk-ing for: ${afk_scoreboard.getScore(caller)} minutes`,
      );
    } else {
...
bridge.bedrockCommands.registerAdminCommand(
  ...
  (caller) => {
    ...
    caller.sendMessage(
      `§o${afkers.length} afkers detected: §r${afkers.filter((p) => p.name).join(", ")}`,
    );
  },
  ...
  (caller, target) => {
    ...
    if (player) {
      if (afk_scoreboard.hasParticipant(player)) {
        caller.sendMessage(
          `§o${player.name} You has been afk-ing for: ${afk_scoreboard.getScore(player)} minutes`,
        );
      } else {
