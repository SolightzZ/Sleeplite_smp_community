...
const stepScoreboard =
  world.scoreboard.getObjective("esploratori:steps") ??
  world.scoreboard.addObjective("esploratori:steps", "steps");

bridge.bedrockCommands.registerCommand(
  "steps-count",
  (player) => {
    player.sendMessage(
      `§eYou have been walking for §o${stepScoreboard.getScore(player)}§r§e blocks`,
    );
  },
  ...
  (player, target) => {
    ...
    if (targetPlayer) {
      system.run(() => {
        ...
        player.sendMessage(
          `§eStep counter score reset to 0 for §o${targetPlayer.name}§r.`,
        );
      });
    } else {
...
system.runInterval(() => {
  for (const player of world.getAllPlayers()) {
    ...
    if (player_dimensions.get(player.id) === player.dimension.id) {
      ...
      if (dist > 1) stepScoreboard.addScore(player, Math.floor(dist));
      else continue; // don't reset old location
    } else {
      // if dimension changes don't update the steps
...
world.afterEvents.playerSpawn.subscribe((e) => {
  ...
  if (e.initialSpawn) {
    ...
    if (
      ...
      !stepScoreboard.hasParticipant(e.player.scoreboardIdentity)
    ) {
