...
var show_mode =
  world.getDynamicProperty("esploratori:nicerchat:show_mode") ?? modes.nickname;
  ...
  (player, mode) => {
    if (mode?.toString() in modes) {
      ...
      world.setDynamicProperty(
        "esploratori:nicerchat:show_mode",
        mode.toString(),
      );
      player.sendMessage("§eChat mode changed to " + mode);
    } else {
      player.sendMessage(
        `§cMode not supported, please use one of: '${Object.values(modes).join("', '")}'. Usage: changeChatNameMode <mode>`,
      );
    }
  },
