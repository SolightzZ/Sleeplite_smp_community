...
bridge.bedrockCommands.registerAdminCommand(
  ...
  (user, target) => {
    ...
    if (target?.readPlayer())
      //readPlayer converts the string in a player object with that nametag
      user.teleport(target.readPlayer().location);
    else {
      user.sendMessage("§ctarget player not found");
    }
  },
  "teleport to the target player location. Usage: tp <username>",
); //this is the description which will be visualised in !help
  ...
  (user, target, time) => {
    ...
    if (target_player) {
      if (time) {
        ...
        //didn't say the time... so forever ig?
        target_player.mute();
        target_player.sendMessage("§oYou have been muted by an admin.");
      }
    } else {
      //he didn't say the nametag
      user.sendMessage(
        "§ctarget player not found. Make sure to use a valid nametag when you run this command.",
      );
    }
  },
...
bridge.bedrockCommands.registerAdminCommand(
  ...
  (user, target) => {
    ...
    if (target_player) {
      ...
      user.sendMessage(
        "§cTarget player not found. Make sure to use a valid username when you run this command.",
      );
    }
  },
...
bridge.bedrockCommands.registerAdminCommand(
  "gamemode",
  async (user, target, mode) => {
    const target_player = target?.readPlayer();
    if (mode.toString() in GameMode && target_player) {
      await target_player.runCommand(`gamemode ${mode} @s`);
      target_player.sendMessage(
        `§aYour gamemode has been changed to ${mode} by an admin.`,
      );
      user.sendMessage("§eCommand succefully executed.");
    } else {
      user.sendMessage(
        "§eMissing parameter. Usage: gamemode <username> <gamemode>",
      );
    }
  },
...
const die_score_names = {
  x: "esploratori:die_loc_x",
  y: "esploratori:die_loc_y",
  z: "esploratori:die_loc_z",
};
const die_loc_x =
  world.scoreboard.getObjective(die_score_names.x) ??
  world.scoreboard.addObjective(die_score_names.x, "player death x");
const die_loc_y =
  world.scoreboard.getObjective(die_score_names.y) ??
  world.scoreboard.addObjective(die_score_names.y, "player death y");
const die_loc_z =
  world.scoreboard.getObjective(die_score_names.z) ??
  world.scoreboard.addObjective(die_score_names.z, "player death z");
...
bridge.bedrockCommands.registerTagCommand(
  ...
  (player) => {
    if (
      ...
      die_loc_x.hasParticipant(player.scoreboardIdentity)
    ) {
      //we check just one, no need for the whole 3 as they are set together
      system.run(() => {
        player.teleport({
          x: die_loc_x.getScore(player.scoreboardIdentity),
          y: die_loc_y.getScore(player.scoreboardIdentity),
          z: die_loc_z.getScore(player.scoreboardIdentity),
        });
        player.sendMessage(
          "§eYou have been teleported to the last available death location.",
        );
      });
    } else {
      player.sendMessage(
        "§cTeleport was not possible: last death location is not available.",
      );
    }
  },
