...
const TPSscoreboard =
  world.scoreboard.getObjective("esploratori:serverstats") ??
  world.scoreboard.addObjective("esploratori:serverstats", "Server Statistics");
...
bridge.bedrockCommands.registerAdminCommand(
  ...
  (user) => {
    world.scoreboard.setObjectiveAtDisplaySlot(DisplaySlotId.Sidebar, {
      objective: TPSscoreboard,
    });
    user.sendMessage("§eServer added to sidebar.");
  },
  "show real-time TPS on players' screen sidebar.",
); //this is the description which will be visualised in !help
...
bridge.bedrockCommands.registerAdminCommand(
  ...
  "hide TPS stats from players' screen.",
); //this is the description which will be visualised in !help
...
function report() {
  ...
  if (
    ...
    bridgeDirect.ready &&
    current - last_report > discord_delay
  ) {
    const players = world.getAllPlayers();
    if (players.length > 0) {
      // if no player is online, no notification is sent
      const message =
        `A TPS drop has been detected on your server!\n\n- TPS: \`${Math.round(counters[types.tps])}\`\n- Entities: \`${counters[types.players] + counters[types.items] + counters[types.mobs]}\`\n- Players: ` +
        players
          .map(
            (player) =>
              `\`${player.name}\` (${player.dimension.getEntities({ maxDistance: 64, location: player.location }).length})`,
          )
          .join(", ");
...
system.runInterval(() => {
  counters[types.tps] =
    (1000 * (system.currentTick - last_tick)) / (Date.now() - last_check);
  last_check = Date.now();
