...
system.runTimeout(() => {
  if (!bridgeDirect.ready) {
    console.error(
      "(BedrockBridge) [simpleCommandLog] Failed to start directBridge. Commands are not being reported.",
    );
    ENABLED = false;
    ...
    try {
      ...
      if (res.formValues[0] !== 0) {
        rawCommand = rawCommand.replace(
          "@t",
          this.playerList[res.formValues[0]],
        );
      }
      ...
      if (forbiddenCommands.includes(command)) {
        player.sendMessage(
          "§cYou are not allowed to run this command. This incident will be reported.",
        );
...
bridge.bedrockCommands.registerTagCommand(
  ...
  (player) => {
    if (ENABLED) {
      ...
      player.sendMessage(
        "§cThis functionality is not currently active. If the error persists please get in touch with assistance.",
      );
    }
  },
  "run a command",
