...
async function importPlugin(pluginPath) {
  try {
    ...
    const normPath = pluginPath.startsWith("./")
      ? pluginPath
      : "./" + pluginPath;
...
system.runTimeout(async () => {
  ...
  console.log(
    `BedrockBridge Plugin Manager: Loaded ${successCount} plugins. Failed: ${failCount}`,
  );
}, 20);
  ...
  pluginDetailForm.show(player).then((response) => {
    ...
    switch (response.selection) {
      case 0: // Toggle Enable/Disable
        system.runTimeout(
          () => togglePluginState(player, plugins, pluginIndex, plugin),
          5,
        );
        break;
      ...
      case 2: // Remove Plugin
        system.runTimeout(
          () => confirmPluginRemoval(player, plugins, pluginIndex, plugin),
          5,
        );
        break;
    ...
    if (wasEnabled) {
      ...
      importPlugin(newPath).then((result) => {
        if (result.success) {
          ...
          player.sendMessage(
            `§ePlugin '${newPath}' failed to load: ${result.error}`,
          );
        }

        system.runTimeout(
          () => showPluginDetailsUI(player, plugins[pluginIndex]),
          5,
        );
      });
    } else {
...
async function togglePluginState(player, plugins, pluginIndex, plugin) {
  ...
  if (plugins[pluginIndex].enabled) {
    ...
    if (result.success) {
      player.sendMessage(
        `§aPlugin '${getPluginName(plugin.path)}' enabled successfully!`,
      );
    } else {
      player.sendMessage(
        `§ePlugin '${getPluginName(plugin.path)}' marked as enabled but failed to load: ${result.error}`,
      );
    }
  } else {
    player.sendMessage(
      `§aPlugin '${getPluginName(plugin.path)}' disabled. Note: Already loaded code will remain in memory until server restart.`,
    );
  }
...
async function confirmPluginRemoval(player, plugins, pluginIndex, plugin) {
  const confirmForm = new MessageFormData()
    ...
    .body(
      `Are you sure you want to remove the plugin '${getPluginName(plugin.path)}'?`,
    )
    .button1("Yes, Remove")
    .button2("Cancel");
  ...
  confirmForm.show(player).then((response) => {
    ...
    if (response.selection === 0) {
      ...
      pluginsDB.set("plugins", plugins);
      player.sendMessage(
        `§aPlugin '${getPluginName(plugin.path)}' removed from plugin manager.`,
      );
      system.runTimeout(() => showPluginListUI(player), 5);
    } else {
      system.runTimeout(() => showPluginDetailsUI(player, plugin), 5);
...
async function showAddPluginUI(player) {
  const addPluginForm = new ModalFormData()
    ...
    .textField("Plugin Path", "e.g. ./bridgePlugins/myPlugin", {
      defaultValue: "./bridgePlugins/",
    });
  ...
  addPluginForm.show(player).then((response) => {
    ...
    pluginsDB.set("plugins", plugins);
    player.sendMessage(
      `§aPlugin '${getPluginName(pluginPath)}' added to plugin manager.`,
    );
...
async function askEnableNewPlugin(player, pluginPath) {
  ...
  if (response.selection === 0) {
    ...
    if (result.success) {
      player.sendMessage(
        `§aPlugin '${getPluginName(pluginPath)}' enabled successfully!`,
      );
    } else {
      player.sendMessage(
        `§ePlugin '${getPluginName(pluginPath)}' marked as enabled but failed to load: ${result.error}`,
      );
    }
  }
...
async function reloadPlugins(player) {
  ...
  player.sendMessage(
    `§aReloaded ${successCount} plugins. §c${failCount} failed.`,
  );
...
const pluginCommand = bridge.bedrockCommands.registerAdminCommand(
  "plugin",
  (player, arg1, arg2) => {
    ...
    switch (action) {
      ...
      default:
        player.sendMessage(
          "§cInvalid command! Use !plugin ui|list|enable|disable|add|remove|reload",
        );
    }
  },
...
function listPlugins(player) {
  ...
  plugins.forEach((plugin) => {
    const status = plugin.enabled ? "§a[ENABLED]" : "§c[DISABLED]";
    player.sendMessage(
      `${status} §f${getPluginName(plugin.path)} (${plugin.path})`,
    );
  });
}
...
function disablePlugin(player, pluginPath) {
  ...
  pluginsDB.set("plugins", plugins);
  player.sendMessage(
    `§aPlugin '${getPluginName(pluginPath)}' disabled. Note: Already loaded code will remain in memory until server restart.`,
  );
}
...
function addPlugin(player, pluginPath) {
  ...
  pluginsDB.set("plugins", plugins);
  player.sendMessage(
    `§aPlugin '${getPluginName(pluginPath)}' added to plugin manager. Use '!plugin enable ${pluginPath}' to enable it.`,
  );
}
...
function removePlugin(player, pluginPath) {
  ...
  pluginsDB.set("plugins", plugins);
  player.sendMessage(
    `§aPlugin '${getPluginName(pluginPath)}' removed from plugin manager.`,
  );
}
...
bridge.bedrockCommands.registerAdminCommand(
  "resetPluginManagment",
  (player) => {
    pluginsDB.clear();
    player.sendMessage(
      "All plugin records have been deleted. Please run reload in order to initialize again then default plugins",
    );
  },
