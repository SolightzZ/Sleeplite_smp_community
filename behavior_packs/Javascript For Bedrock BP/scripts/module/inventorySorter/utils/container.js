...
const buildStackKey = (item) => {
   ...
   const durStr = dur ? `${dur.damage ?? 0}/${dur.maxDurability ?? 0}` : '';

   if (!item.nameTag && !loreStr && !enchStr && !durStr) return item.typeId;

   return `${item.typeId}\x00${item.nameTag || ''}\x00${loreStr}\x00${enchStr}\x00${durStr}`;
};
