...
import { getItemMaterialGroup } from '../utils/item.js';
...
export function sortBlockContainer(player, mode) {
   ...
   const emptySlots = container.size - itemCount;

   if (itemCount === 0) {
      return { ok: true, msg: msgContainerEmpty };
   }

   const merged = _sortByMode(rawItems, sortMode, container.size, container, 0);
   if (merged === null) {
      return { ok: true, msg: msgSorted };
   }
