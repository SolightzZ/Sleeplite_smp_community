...
export function sortPlayerInventory(player, mode) {
  ...
  const hotbarItems = Array.from({ length: hotbarSize }, (_, i) =>
    inv.getItem(i),
  );
  const mainItems = Array.from({ length: invSize - hotbarSize }, (_, i) =>
    inv.getItem(i + hotbarSize),
  );

  const hotbarDone =
    isContainerSorted(inv, sortMode) || hotbarItems.every((item) => !item);
  ...
  const newHotbar = hotbarDone
    ? hotbarItems
    : sortAndMergeItems(hotbarItems, hotbarSize).sort((a, b) =>
        compareItemsByMode(a, b, sortMode),
      );
  const newMain = mainDone
    ? mainItems
    : sortAndMergeItems(mainItems, invSize - hotbarSize).sort((a, b) =>
        compareItemsByMode(a, b, sortMode),
      );
...
export function sortBlockContainer(player, mode) {
  ...
  if (sortMode === "chess") {
    const sorted = sortAndMergeItems(items, items.length).sort((a, b) =>
      compareItemsByMode(a, b, "type"),
    );
    ...
    merged = sortAndMergeItems(items, size).sort((a, b) =>
      compareItemsByMode(a, b, sortMode),
    );
  }
