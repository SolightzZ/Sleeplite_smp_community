import { world, system, PlayerInteractWithBlockBeforeEvent } from "@minecraft/server";
    ...
    constructor() {
        ...
        this.settings = {
            ...
            enableDurability: {
                ...
                tooltip: "If enabled, the hoe used will take durability damage for each block tilled.",
            },
        };
        ...
        this.validBlocks = new Set(["minecraft:dirt", "minecraft:grass_block", "minecraft:coarse_dirt", "minecraft:rooted_dirt", "minecraft:dirt_with_roots"]);
    }
        ...
        system.run(() => {
            ...
            for (let x = -halfSize; x <= halfSize; x++) {
                for (let z = -halfSize; z <= halfSize; z++) {
                    const hoe = player.getComponent("inventory").container.getItem(player.selectedSlotIndex);
                    ...
                    if (currentBlock && this.validBlocks.has(currentBlock.typeId) && currentBlock.above(1).isAir) {
                        currentBlock.setType("minecraft:farmland");
                        if (this.enableDurability && hoe.getComponent("durability")) CTsAPI.utils.increaseDurability(player, hoe, 1);
                    }
                }
            }
