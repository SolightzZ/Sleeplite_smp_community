...
import { JOBS_ADDON_CONFIG, JOB_DEFINITIONS } from "../config/jobs_config.js";
...
const I18N = {
  en: {
    ...
    "limit.reached":
      "§c[!] Activity limit reached for {job}§c! §7Regenerates over time.",
  ...
  es: {
    ...
    "job.joined":
      "§a* Te uniste a {job}§a. §7Escribe {command} para abrir el menú.",
    "job.left": "§6Dejaste {job}§6. §7Enfriamiento: 5 minutos",
    "limit.reached":
      "§c[!] Límite de actividad alcanzado para {job}§c. §7Se regenera con el tiempo.",
    levelup: "§6* §l¡SUBISTE DE NIVEL!§r §6{job} §ealcanzó el nivel {level}!",

    "quests.new":
      "§e¡Hay nuevas misiones diarias! §7Revisa tu menú de trabajos.",
  ...
  pt_BR: {
    ...
    "job.joined":
      "§a* Você entrou em {job}§a! §7Digite {command} para abrir o menu.",
    "job.left": "§6Você saiu de {job}§6. §7Recarga: 5 minutos",
    "limit.reached":
      "§c[!] Limite de atividade atingido para {job}§c! §7Regenera com o tempo.",
  ...
  ru: {
    ...
    "job.joined":
      "§a* Вы выбрали {job}§a! §7Введите {command}, чтобы открыть меню.",
    "job.left": "§6Вы покинули {job}§6. §7КД: 5 минут",
    "limit.reached":
      "§c[!] Достигнут лимит активности для {job}§c. §7Восстанавливается со временем.",
  ...
  ja: {
    ...
    "job.joined":
      "§a* {job}§a に参加しました！ §7{command} でメニューを開けます。",
    "job.left": "§6{job}§6 を退出しました。§7クールダウン：5分",
    "limit.reached":
      "§c[!] {job}§c のアクティビティ上限に達しました。§7時間で回復します。",
    levelup: "§6* §lレベルアップ！§r §6{job} §eがレベル {level} に到達！",

    "quests.new":
      "§e新しいデイリークエストがあります！§7メニューを確認してください。",
  ...
  de: {
    ...
    "job.joined":
      "§a* {job}§a beigetreten! §7Tippe {command}, um das Menü zu öffnen.",
    "job.left": "§6{job}§6 verlassen. §7Abklingzeit: 5 Minuten",
    "limit.reached":
      "§c[!] Aktivitätslimit erreicht für {job}§c! §7Regeneriert mit der Zeit.",
    ...
    "achievement.unlocked":
      "§6* §lERRUNGENSCHAFT FREIGESCHALTET!§r §6{achievement}",
  ...
  fr: {
    ...
    "job.joined":
      "§a* Vous avez rejoint {job}§a ! §7Tapez {command} pour ouvrir le menu.",
    "job.left": "§6Vous avez quitté {job}§6. §7Recharge : 5 minutes",
    "limit.reached":
      "§c[!] Limite d’activité atteinte pour {job}§c. §7Se régénère avec le temps.",
    levelup:
      "§6* §lNIVEAU SUPÉRIEUR !§r §6{job} §ea atteint le niveau {level} !",

    "quests.new":
      "§eNouvelles quêtes quotidiennes disponibles ! §7Vérifiez le menu.",
  ...
  ko: {
    ...
    "job.joined":
      "§a* {job}§a 에 참여했습니다! §7{command} 명령어로 메뉴를 여세요.",
    "job.left": "§6{job}§6 을(를) 나갔습니다. §7쿨다운: 5분",
    "limit.reached":
      "§c[!] {job}§c 활동 제한에 도달했습니다. §7시간이 지나면 회복됩니다.",
  ...
  id: {
    ...
    "job.joined":
      "§a* Bergabung dengan {job}§a! §7Ketik {command} untuk membuka menu.",
    "job.left": "§6Keluar dari {job}§6. §7Cooldown: 5 menit",
    "limit.reached":
      "§c[!] Batas aktivitas tercapai untuk {job}§c! §7Pulih seiring waktu.",
...
function categoryName(player, category) {
  const lang = getPlayerLang(player);
  return (
    (CATEGORY_I18N[lang] || CATEGORY_I18N[LANG.DEFAULT])?.[category] || category
  );
}
...
const CONFIG = {
  // Core Settings
  OBJECTIVE_ID: JOBS_ADDON_CONFIG.rewards.defaultScoreObjective || "money",
  MAX_JOBS: 3,
  MAX_LEVEL: 100,
  ...
  ENABLE_LEADERBOARD:
    JOBS_ADDON_CONFIG.leaderboard?.enableSidebarMoney ?? false,
  ...
  REQUIRE_SNEAK_FOR_CLOCK_MENU:
    JOBS_ADDON_CONFIG.menu.requireSneakForClockMenu ?? true,
...
const MENU_COMMAND = `/${JOBS_ADDON_CONFIG.menu.command.name}`;
...
function getRewardActions(source, jobId = null) {
  ...
  const defaults = Array.isArray(rewardsConfig[source])
    ? rewardsConfig[source]
    : [];
...
function addScoreReward(player, objectiveId, amount) {
  if (!player?.scoreboardIdentity || amount === 0) return 0;
  try {
    let objective = world.scoreboard.getObjective(objectiveId);
    if (!objective)
      objective = world.scoreboard.addObjective(objectiveId, objectiveId);
    const current = objective.getScore(player.scoreboardIdentity) || 0;
    objective.setScore(player.scoreboardIdentity, current + amount);
    return amount;
  } catch (error) {
    console.error("[Jobs] Failed to apply score reward:", error);
    return 0;
  }
}
...
function applyConfiguredRewardPayouts(player, source, context = {}) {
  ...
  for (const action of actions) {
    ...
    if (action.type === "score") {
      const objective = action.objective || CONFIG.OBJECTIVE_ID;
      const awarded = addScoreReward(player, objective, amount);
      if (awarded > 0)
        payouts.push({ type: "score", objective, amount: awarded });
      continue;
    }
    ...
    if (action.type === "item") {
      ...
      if (awarded > 0)
        payouts.push({ type: "item", itemId: action.itemId, amount: awarded });
      continue;
    }
...
function formatRewardPart(payout) {
  ...
  if (payout.type === "score") {
    return payout.objective === CONFIG.OBJECTIVE_ID
...
function openJobsMenu(player) {
  system.run(() => {
    if (!player) return;
    if (!hasPlayerLang(player))
      jobsSystem.gui.showLanguageMenu(player, { returnToMain: true });
...
function getDefaultNotificationSettings() {
  return {
    ...
    rewardActionBar:
      JOBS_ADDON_CONFIG.notifications?.defaults?.rewardActionBar ?? true,
    questMessages:
      JOBS_ADDON_CONFIG.notifications?.defaults?.questMessages ?? true,
    achievementMessages:
      JOBS_ADDON_CONFIG.notifications?.defaults?.achievementMessages ?? true,
    levelUpMessages:
      JOBS_ADDON_CONFIG.notifications?.defaults?.levelUpMessages ?? true,
    salaryMessages:
      JOBS_ADDON_CONFIG.notifications?.defaults?.salaryMessages ?? true,
...
function matchesConfiguredUpgrade(itemType, match = null) {
  ...
  const includesAny = Array.isArray(match.includesAny)
    ? match.includesAny.map((value) => String(value).toLowerCase())
    : [];
  const equalsAny = Array.isArray(match.equalsAny)
    ? match.equalsAny.map((value) => String(value).toLowerCase())
    : [];
  if (equalsAny.length > 0 && equalsAny.includes(source)) return true;
  if (
    includesAny.length > 0 &&
    includesAny.some((value) => source.includes(value))
  )
    return true;
  ...
  for (const [key, value] of Object.entries(definitions || {})) {
    ...
    normalized[key.toUpperCase()] = {
      ...
      upgrades: Array.isArray(value.upgrades)
        ? value.upgrades.map(normalizeUpgrade)
        : [],
    };
  }
  return normalized;
...
const REWARDS = {
  BLOCKS: {
    ...
    "minecraft:deepslate_lapis_ore": 18,
    "minecraft:diamond_ore": 50,
    "minecraft:deepslate_diamond_ore": 65,
    "minecraft:emerald_ore": 60,
    ...
    "minecraft:amethyst_block": 25,
    "minecraft:budding_amethyst": 60,
    "minecraft:sculk": 10,
    "minecraft:sculk_catalyst": 30,
    "minecraft:sculk_sensor": 35,
    "minecraft:sculk_shrieker": 45,

    // Utility
    "minecraft:obsidian": 12,
    "minecraft:crying_obsidian": 18,
    "minecraft:glowstone": 6,
    "minecraft:sea_lantern": 10,
    "minecraft:copper_block": 15,
    "minecraft:exposed_copper": 12,
    "minecraft:weathered_copper": 18,
    "minecraft:oxidized_copper": 20,
  },
  ...
  FARMING: {
    ...
    "minecraft:melon_block": 5,
    "minecraft:pumpkin": 5,
    "minecraft:sugar_cane": 2,
    "minecraft:bamboo": 2,
    "minecraft:cactus": 3,
    "minecraft:kelp": 1,
    "minecraft:sweet_berry_bush": 4,
    "minecraft:cocoa": 4,
  ...
  CRAFTING: {
    ...
    "minecraft:iron_hoe": 15,
    "minecraft:iron_sword": 15,
    "minecraft:diamond_pickaxe": 50,
    "minecraft:diamond_axe": 50,
    "minecraft:diamond_shovel": 50,
    "minecraft:diamond_hoe": 50,
    "minecraft:diamond_sword": 50,
    "minecraft:netherite_pickaxe": 250,
    "minecraft:netherite_axe": 250,
    "minecraft:netherite_shovel": 250,
    "minecraft:netherite_hoe": 250,
    "minecraft:netherite_sword": 250,

    // Armor
    "minecraft:leather_helmet": 6,
    "minecraft:leather_chestplate": 10,
    "minecraft:leather_leggings": 8,
    "minecraft:leather_boots": 5,
    "minecraft:iron_helmet": 20,
    "minecraft:iron_chestplate": 35,
    "minecraft:iron_leggings": 30,
    "minecraft:iron_boots": 15,
    "minecraft:diamond_helmet": 80,
    "minecraft:diamond_chestplate": 130,
    "minecraft:diamond_leggings": 110,
    "minecraft:diamond_boots": 60,
    "minecraft:netherite_helmet": 400,
    "minecraft:netherite_chestplate": 650,
    "minecraft:netherite_leggings": 550,
    "minecraft:netherite_boots": 300,
    ...
    "minecraft:piston": 12,
    "minecraft:sticky_piston": 20,
    "minecraft:observer": 15,
    "minecraft:repeater": 8,
    "minecraft:comparator": 12,
    "minecraft:enchanting_table": 150,
    "minecraft:anvil": 60,
    "minecraft:grindstone": 20,
    "minecraft:beacon": 800,
    "minecraft:end_crystal": 200,
    "minecraft:conduit": 250,
    "minecraft:lodestone": 100,

    // Redstone
    "minecraft:redstone_block": 35,
    "minecraft:daylight_detector": 15,
    "minecraft:target": 12,
    "minecraft:lightning_rod": 18,
  },
...
const QUESTS = {
  miner: [
    ...
    },
    {
      ...
      reward: 300,
      type: "block",
      items: ["minecraft:iron_ore", "minecraft:deepslate_iron_ore"],
    },
    {
      id: "mine_diamond",
      name: "Diamond Hunter",
      ...
      reward: 800,
      type: "block",
      items: ["minecraft:diamond_ore", "minecraft:deepslate_diamond_ore"],
    },
    {
  ...
  hunter: [
    {
      ...
      reward: 200,
      type: "mob",
      mobs: ["minecraft:zombie", "minecraft:husk", "minecraft:drowned"],
    },
    {
      id: "kill_skeletons",
      name: "Bone Collector",
      ...
      reward: 180,
      type: "mob",
      mobs: ["minecraft:skeleton", "minecraft:stray"],
    },
    {
      id: "kill_creepers",
      name: "Creeper Hunter",
    ...
    },
    {
      ...
      reward: 1500,
      type: "boss",
      mobs: ["minecraft:ender_dragon", "minecraft:wither"],
    },
    {
      id: "kill_warden",
      name: "Deep Dark Terror",
      target: 1,
      reward: 2000,
      type: "mob",
  ...
  farmer: [
    ...
    },
    {
      ...
      reward: 100,
      type: "crop",
      items: ["minecraft:melon_block", "minecraft:pumpkin"],
    },
    {
      id: "breed_animals",
      name: "Animal Breeder",
...
const ACHIEVEMENTS = {
  FIRST_JOB: {
    id: "first_job",
    name: "First Step",
    description: "Join your first job",
    reward: 100,
  },
  JACK_OF_ALL_TRADES: {
    id: "jack_of_all",
    name: "Jack of All Trades",
    description: "Join all 12 available jobs",
    reward: 2000,
  },
  MILLIONAIRE: {
    id: "millionaire",
  ...
  MAX_LEVEL: {
    ...
    description: "Reach level 100 in any job",
    reward: 3000,
  },
  QUEST_MASTER: {
    id: "quest_master",
  ...
  UPGRADE_EXPERT: {
    id: "upgrade_expert",
    name: "Upgrade Expert",
    description: "Unlock 20 job upgrades",
    reward: 2000,
  },
  BLOCK_BREAKER: {
    id: "block_breaker",
...
function isValidResourceId(id) {
  return (
    typeof id === "string" && id.includes(":") && id.split(":").length === 2
  );
}
...
function isToolItem(typeId) {
  return ["pickaxe", "axe", "shovel", "hoe", "sword"].some((token) =>
    typeId.includes(token),
  );
}

function isArmorItem(typeId) {
  return ["helmet", "chestplate", "leggings", "boots"].some((token) =>
    typeId.includes(token),
  );
}
...
function isRedstoneItem(typeId) {
  return [
    ...
    "dispenser",
    "dropper",
  ].some((token) => typeId.includes(token));
}
...
function getBlacksmithReward(typeId) {
  ...
  if (isArmorItem(typeId))
    return Math.max(8, Math.floor((REWARDS.CRAFTING[typeId] || 20) * 0.75));
  if (isToolItem(typeId))
    return Math.max(6, Math.floor((REWARDS.CRAFTING[typeId] || 12) * 0.6));
  return 0;
}
...
function isTraderEntity(typeId) {
  return (
    typeId === "minecraft:villager" || typeId === "minecraft:wandering_trader"
  );
}
...
class DataManager {
  ...
  init() {
    ...
    if (CONFIG.ENABLE_QUESTS) {
      this.startQuestReset();
    }

    if (CONFIG.JOB_LIMIT_ENABLED) {
      this.startLimitRegeneration();
    }
    ...
    world.beforeEvents.playerLeave?.subscribe((e) => {
      try {
        const data = this.playerData.get(e.player.id);
        if (data) {
          this.saveDataToStorage(e.player, data);
        }
      } catch (error) {
        console.error("[Jobs] Failed to save player data before leave:", error);
      }
    });

    world.afterEvents.playerLeave?.subscribe((e) => {
      this.offlinePlayers.set(e.playerId, system.currentTick);
      this.dirtyPlayers.delete(e.playerId);
    });
  }
  ...
  setupObjectives() {
    system.run(() => {
      try {
        let objective = world.scoreboard.getObjective(CONFIG.OBJECTIVE_ID);
        if (!objective) {
  ...
  markDirty(playerOrId) {
    ...
    if (playerId) this.dirtyPlayers.add(playerId);
  }
  ...
  loadPlayerData(player) {
    try {
      ...
      if (questData) {
        ...
        if (parsed) {
          this.questProgress.set(
            player.id,
            new Map(Object.entries(parsed.progress || {})),
          );
      ...
      data.history = normalizeHistoryEntries(data.history || []).slice(
        0,
  ...
  migrateData(oldData) {
    ...
    if (oldData.jobs)
      newData.jobs = new Set(
        Array.isArray(oldData.jobs) ? oldData.jobs : Object.keys(oldData.jobs),
      );
    if (oldData.historicalJobs)
      newData.historicalJobs = new Set(
        Array.isArray(oldData.historicalJobs)
          ? oldData.historicalJobs
          : Object.keys(oldData.historicalJobs),
      );
    else newData.historicalJobs = new Set(newData.jobs);
    if (oldData.joinTime)
      newData.joinTime = new Map(Object.entries(oldData.joinTime));
    if (oldData.earnings)
      newData.earnings = new Map(Object.entries(oldData.earnings));
    ...
    if (oldData.unlockedUpgrades)
      newData.unlockedUpgrades = new Map(
        Object.entries(oldData.unlockedUpgrades),
      );
    if (oldData.explorerCooldowns)
      newData.explorerCooldowns = new Map(
        Object.entries(oldData.explorerCooldowns),
      );
    ...
    newData.settings = {
      notifications: normalizeNotificationSettings(
        oldData.settings?.notifications || oldData.notifications || {},
      ),
    };
  ...
  saveDataToStorage(playerOrId, dataOverride = null) {
    try {
      ...
      const saveData = {
        ...
        explorerCooldowns: Object.fromEntries(
          data.explorerCooldowns || new Map(),
        ),
        ...
        history: normalizeHistoryEntries(data.history || []).slice(
          0,
          CONFIG.HISTORY_MAX_ENTRIES,
        ),
        version: 4,
  ...
  saveAllData(force = false) {
    for (const player of world.getAllPlayers()) {
      if (force || this.dirtyPlayers.has(player.id)) {
  ...
  cleanup() {
    ...
    for (const [playerId, leftAtTick] of this.offlinePlayers) {
      if (
        ...
        currentTick - leftAtTick >= CONFIG.CLEANUP_INTERVAL
      ) {
  ...
  startQuestReset() {
    system.runInterval(() => {
      for (const player of world.getAllPlayers()) {
        ...
        if (this.shouldNotify(player, "systemTips"))
          player.sendMessage(t(player, "quests.new"));
      ...
      for (const player of world.getAllPlayers()) {
        ...
        for (const jobId of data.jobs) {
          ...
          if (current > 0) {
            limits.current.set(
              jobId,
              Math.max(0, current - CONFIG.LIMIT_REGEN_RATE),
            );
            changed = true;
          }
        }
  ...
  canJoinJob(player, jobId) {
    ...
    if (data.jobs.has(jobId))
      return { canJoin: false, reason: "§cYou already have this job!" };
    if (data.jobs.size >= CONFIG.MAX_JOBS)
      return {
        canJoin: false,
        reason: `§cMaximum ${CONFIG.MAX_JOBS} jobs reached!`,
      };
    ...
    if (this.cooldowns.has(cooldownKey)) {
      ...
      if (remaining > 0) {
        return {
          canJoin: false,
          reason: `§cCooldown: ${Math.ceil(remaining / 1200)} minutes remaining`,
        };
      }
  ...
  leaveJob(player, jobId) {
    ...
    this.cooldowns.set(
      `${player.id}_${jobId}`,
      system.currentTick + CONFIG.COOLDOWN_TICKS,
    );
  ...
  addReward(player, jobId, baseAmount, itemType = null) {
    ...
    if (CONFIG.JOB_LIMIT_ENABLED) {
      ...
      if (!limitCheck.canPerform) {
        if (Math.random() < 0.1 && this.shouldNotify(player, "systemTips"))
          player.sendMessage(
            t(player, "limit.reached", { job: jobNameColored(player, jobId) }),
          );
    ...
    for (const upgradeName of upgrades) {
      ...
      if (upgrade && upgrade.condition(itemType || ""))
    ...
    const finalAmount = Math.max(
      1,
      Math.floor(baseAmount * levelBonus * multiplier),
    );
    ...
    if (
      data.totalEarnings >= 1000000 &&
      !this.hasAchievement(player, ACHIEVEMENTS.MILLIONAIRE)
    ) {
    ...
    if (payoutResult.points > 0 || payoutResult.payouts.length > 0) {
      this.addHistoryEntry(player, {
        ...
        detail:
          formatRewardSummary(payoutResult) ||
          `${formatNumber(finalAmount)} points`,
      });
    }
    return payoutResult;
  }
  ...
  addXP(player, jobId, amount) {
    ...
    if (newXP >= xpNeeded && currentLevel < CONFIG.MAX_LEVEL) {
      ...
      if (this.shouldNotify(player, "levelUpMessages")) {
        player.sendMessage(
          t(player, "levelup", {
            job: jobNameColored(player, jobId),
  ...
  updateScoreboard(player, amount) {
    try {
      const objective = world.scoreboard.getObjective(CONFIG.OBJECTIVE_ID);
      if (objective && player.scoreboardIdentity) {
        objective.setScore(
          player.scoreboardIdentity,
          (objective.getScore(player.scoreboardIdentity) || 0) + amount,
        );
      }
    } catch (error) {
      console.error("[Jobs] Scoreboard update error:", error);
    }
  }
  ...
  getAvailableUpgrades(player, jobId) {
    ...
    const job = JOBS[jobId.toUpperCase()];
    if (!job) return [];
    return job.upgrades.filter(
      (u) => level >= u.level && !unlocked.includes(u.name),
    );
  }
  ...
  unlockUpgrade(player, jobId, upgradeName) {
    const data = this.getPlayerData(player);
    if (!data)
      return { success: false, reason: "Missing player data", cost: 0 };

    const upgrades = data.unlockedUpgrades.get(jobId) || [];
    if (upgrades.includes(upgradeName))
      return { success: false, reason: "Already unlocked", cost: 0 };
    ...
    if (CONFIG.UPGRADE_COSTS_ENABLED) {
      const currentBalance =
        world.scoreboard
          .getObjective(CONFIG.OBJECTIVE_ID)
          ?.getScore(player.scoreboardIdentity) || 0;
      if (currentBalance < cost) {
        return {
          success: false,
          reason: `You need $${formatNumber(cost)} to unlock this upgrade.`,
          cost,
        };
      }
    ...
    const totalUpgrades = Array.from(data.unlockedUpgrades.values()).reduce(
      (a, b) => a + b.length,
      0,
    );
    if (
      totalUpgrades >= 20 &&
      !this.hasAchievement(player, ACHIEVEMENTS.UPGRADE_EXPERT)
    ) {
    ...
    for (const quest of jobQuests) {
      ...
      switch (quest.type) {
        ...
        case "treasure":
          if (
            type === quest.type &&
            quest.items?.some((item) =>
              itemType?.includes(item.replace("minecraft:", "")),
            )
          )
            shouldProgress = true;
          break;
        ...
        case "boss":
          if (
            type === quest.type &&
            quest.mobs?.some((mob) =>
              itemType?.includes(mob.replace("minecraft:", "")),
            )
          )
            shouldProgress = true;
          break;
        case "trade":
          if (type === "trade" && quest.tier === itemType)
            shouldProgress = true;
          break;
        case "craft":
          if (type === "craft" && quest.category === itemType)
            shouldProgress = true;
          break;
        case "breed":
      ...
      if (newProgress >= quest.target) {
        ...
        if (data) {
          ...
          if (
            data.totalQuests >= 100 &&
            !this.hasAchievement(player, ACHIEVEMENTS.QUEST_MASTER)
          ) {
          ...
          if (this.shouldNotify(player, "questMessages")) {
            player.sendMessage(
              t(player, "quests.complete", { quest: quest.name }),
            );
          }
          if (this.shouldNotify(player, "rewardChat")) {
            for (const line of getRewardLines(payoutResult))
  ...
  unlockAchievement(player, achievement) {
    ...
    if (this.shouldNotify(player, "achievementMessages")) {
      player.sendMessage(
        t(player, "achievement.unlocked", { achievement: achievement.name }),
      );
  ...
  updateSessionStats(player, statType, amount = 1) {
    ...
    if (statType === "blocksBroken") {
      ...
      if (data.lifetimeBlocksBroken >= 10000)
        this.unlockAchievement(player, ACHIEVEMENTS.BLOCK_BREAKER);
    }
    if (statType === "mobsKilled") {
      data.lifetimeMobsKilled = (data.lifetimeMobsKilled || 0) + amount;
      if (data.lifetimeMobsKilled >= 1000)
        this.unlockAchievement(player, ACHIEVEMENTS.MOB_SLAYER);
    }
    if (statType === "blocksPlaced") {
      data.lifetimeBlocksPlaced = (data.lifetimeBlocksPlaced || 0) + amount;
      if (data.lifetimeBlocksPlaced >= 5000)
        this.unlockAchievement(player, ACHIEVEMENTS.MASTER_BUILDER);
    }
  ...
  resetPlayerData(player) {
    ...
    try {
      ...
      console.error("[Jobs] Failed to reset money scoreboard:", error);
    }
...
class JobGUI {
  ...
  async showMainMenu(player) {
    ...
    const jobList =
      jobCount > 0
        ? Array.from(data.jobs)
            .map((j) => jobNameColored(player, j).replace("§r", ""))
            .join("§7, ")
        : "§cNone";
    ...
    const form = new ActionFormData()
      ...
      .button(t(player, "btn.leave_job"), "textures/items/barrier")
      ...
      .button(t(player, "btn.activity"), "textures/items/clock")
      .button(
        "§6Leaderboards\n§7Top earners & levels",
      ...
      .button(
        "§5Notifications\n§7Toggle message noise",
        "textures/items/experience_bottle",
      )
      .button(t(player, "btn.language"), "textures/items/map")
      .button(t(player, "btn.help"), "textures/items/paper")
      .button(t(player, "btn.close"), "textures/items/barrier");
    ...
    try {
      ...
      switch (response.selection) {
        case 0:
          await this.showMyJobs(player);
          break;
        case 1:
          await this.showJoinJob(player);
          break;
        case 2:
          await this.showLeaveJob(player);
          break;
        case 3:
          await this.showStatistics(player);
          break;
        case 4:
          await this.showUpgrades(player);
          break;
        case 5:
          await this.showQuests(player);
          break;
        case 6:
          await this.showAchievements(player);
          break;
        case 7:
          await this.showActivity(player);
          break;
        case 8:
          await this.showLeaderboardMenu(player);
          break;
        case 9:
          await this.showHistoryLog(player);
          break;
        case 10:
          await this.showNotificationSettings(player);
          break;
        case 11:
          await this.showLanguageMenu(player, { returnToMain: true });
          break;
        case 12:
  ...
  async showLanguageMenu(player, options = {}) {
    ...
    for (const lang of LANGUAGES) {
      ...
      form.button(`§f${lang.label}${mark}`, "textures/items/map");
    }
    ...
    try {
      ...
      if (response.selection < max) {
        ...
        player.onScreenDisplay.setActionBar(
          t(player, "ui.language.saved", { lang: selected.label }),
        );
      }
  ...
  async showMyJobs(player) {
    ...
    if (jobArray.length === 0) {
      ...
      for (const jobId of jobArray) {
        ...
        const limitText = CONFIG.JOB_LIMIT_ENABLED
          ? ` §8| §3${stats.limitInfo.remaining} Activity`
          : "";
  ...
  async showJobDetails(player, jobId) {
    ...
    const form = new ActionFormData()
      ...
      .button("§cLeave Job", "textures/items/barrier")
    ...
    try {
      ...
      switch (response.selection) {
        case 0:
          ...
          break;
        case 1:
          await this.showJobQuests(player, jobId);
          break;
        case 2:
          await this.confirmLeaveJob(player, jobId);
          break;
        default:
          await this.showMyJobs(player);
      }
  ...
  async confirmJoinJob(player, jobId) {
    ...
    const form = new MessageFormData()
      .title(`§lJoin ${jobNamePlain(player, jobId)}?`)
      .body(
        `${job.description}\n\n` +
          `§6Benefits:§r\n` +
    ...
    try {
      ...
      if (this.data.shouldNotify(player, "systemTips"))
        player.sendMessage(
          t(player, "job.joined", {
            job: jobNameColored(player, jobId),
      ...
      player.sendMessage(
        `§c${error instanceof Error ? error.message : String(error)}`,
      );
    }
  }
  ...
  async showLeaveJob(player) {
    ...
    const jobArray = Array.from(data.jobs);
    if (jobArray.length === 0) {
      form.button("§cNo Jobs to Leave", "textures/items/barrier");
    } else {
      for (const jobId of jobArray) {
        const stats = this.data.getJobStats(player, jobId);
        form.button(
  ...
  async confirmLeaveJob(player, jobId) {
    ...
    try {
      ...
      if (this.data.shouldNotify(player, "systemTips"))
        player.sendMessage(
          t(player, "job.left", { job: jobNameColored(player, jobId) }),
        );
    } catch (error) {
      player.sendMessage(
        `§c${error instanceof Error ? error.message : String(error)}`,
  ...
  async showJobUpgrades(player, jobId) {
    ...
    if (available.length > 0) {
      ...
      body += available
        .map(
          (u) =>
            `§e• ${u.name} §7(Lv. ${u.level}) §8- §a$${formatNumber(getUpgradeCost(u))}\n  §8${u.bonus}`,
        )
        .join("\n\n");
    } else {
    ...
    if (available.length > 0) {
      for (const upgrade of available) {
        form.button(
          `§e${upgrade.name}\n§7Lv ${upgrade.level} • $${formatNumber(getUpgradeCost(upgrade))}`,
    ...
    try {
      ...
      if (available.length > 0 && response.selection < available.length) {
        await this.confirmUnlockUpgrade(
          player,
          jobId,
          available[response.selection],
        );
      } else {
  ...
  async showUpgrades(player) {
    ...
    const jobArray = Array.from(data.jobs);
    if (jobArray.length === 0) {
      form.button("§cNo Active Jobs", "textures/items/barrier");
    } else {
      for (const jobId of jobArray) {
        const stats = this.data.getJobStats(player, jobId);
        form.button(
    ...
    if (jobArray.length === 0) {
      ...
      for (const jobId of jobArray) {
        ...
        form.button(
          ...
          `textures/items/${JOBS[jobId.toUpperCase()].icon}`,
        );
      }
    }
  ...
  async showJobQuests(player, jobId) {
    ...
    const form = new ActionFormData()
      .title(
        `§l${JOBS[jobId.toUpperCase()].color}${jobNamePlain(player, jobId)} Quests`,
      )
      .body(body)
      .button("§7« Back", "textures/items/arrow");
  ...
  async showLeaderboardMenu(player) {
    ...
    try {
      ...
      if (response.selection === 0)
        return await this.showLeaderboardResults(player, null, "earnings");
      if (response.selection === 1)
        return await this.showJobLeaderboardPicker(player, "earnings");
      if (response.selection === 2)
        return await this.showJobLeaderboardPicker(player, "level");
  ...
  async showJobLeaderboardPicker(player, metric = "earnings") {
    const form = new ActionFormData()
      .title(
        metric === "level"
          ? "§l§bJob Level Leaderboards"
          : "§l§eJob Earnings Leaderboards",
      )
      .body("§7Choose a job:");
    ...
    for (const job of jobEntries) {
      form.button(
        `${job.color}${jobNamePlain(player, job.id)}
§7${metric === "level" ? "Highest levels" : "Highest earners"}`,
        `textures/items/${job.icon}`,
      );
    }
    form.button("§7« Back", "textures/items/arrow");
    ...
    try {
      ...
      if (response.canceled) return;
      if (response.selection < jobEntries.length) {
        return await this.showLeaderboardResults(
          player,
          jobEntries[response.selection].id,
          metric,
        );
      }
  ...
  async showLeaderboardResults(player, jobId = null, metric = "earnings") {
    const entries = jobId
      ? this.data.getJobLeaderboard(
          jobId,
          metric,
          CONFIG.LEADERBOARD_GUI_ENTRIES,
        )
      : this.data.getTopEarners(CONFIG.LEADERBOARD_GUI_ENTRIES);

    let title = "§l§6Top Total Earners";
    if (jobId && metric === "earnings")
      title = `§l${JOBS[jobId.toUpperCase()]?.color || "§e"}${jobNamePlain(player, jobId)} Earners`;
    if (jobId && metric === "level")
      title = `§l${JOBS[jobId.toUpperCase()]?.color || "§b"}${jobNamePlain(player, jobId)} Levels`;
  ...
  async showHistoryLog(player) {
    ...
    if (entries.length === 0) {
      ...
      body += entries
        .map((entry) => {
          const jobPrefix = entry.jobId
            ? `${jobNameColored(player, entry.jobId)} §8• §r`
            : "";
    ...
    const form = new ActionFormData()
      ...
      .body(body)
      .button("§cClear History", "textures/items/barrier")
      .button("§7« Back", "textures/items/arrow");
  ...
  async showHelp(player) {
    const form = new ActionFormData()
      ...
      .body(
        `§6--- Getting Started ---§r\n` +
          `§7• Use §e/jobs:gui §7to open this menu\n` +
          ...
          Object.values(JOBS)
            .map(
              (j) =>
                `${j.color}• ${jobNamePlain(player, j.id)} §7- ${categoryName(player, j.category)}`,
            )
            .join("\n") +
          `\n\n§6--- Tips ---§r\n` +
...
class EventManager {
  ...
  sendRewardChat(player, message) {
    if (message && this.data.shouldNotify(player, "rewardChat"))
  ...
  sendRewardActionBar(player, message, chance = 1) {
    if (
      message &&
      this.data.shouldNotify(player, "rewardActionBar") &&
      Math.random() < chance
    ) {
  ...
  setupEvents() {
    world.afterEvents.playerBreakBlock?.subscribe((e) => {
      try {
        ...
        console.error("[Jobs] Block break error:", error);
      }
    });

    world.afterEvents.playerPlaceBlock?.subscribe((e) => {
      try {
        this.handleBlockPlace(e);
      } catch (error) {
        console.error("[Jobs] Block place error:", error);
      }
    });
    ...
    world.afterEvents.entityDie?.subscribe((e) => {
      try {
        ...
        console.error("[Jobs] Entity death error:", error);
      }
    });
    ...
    world.afterEvents.itemUse?.subscribe((e) => {
      try {
        ...
        console.error("[Jobs] Item use error:", error);
      }
    });
    world.afterEvents.playerInteractWithBlock?.subscribe((e) => {
      try {
        this.handlePlayerInteractWithBlock(e);
      } catch (error) {
        console.error("[Jobs] Block interaction error:", error);
      }
    });

    world.afterEvents.playerInteractWithEntity?.subscribe((e) => {
      try {
        this.handlePlayerInteractWithEntity(e);
      } catch (error) {
        console.error("[Jobs] Entity interaction error:", error);
      }
    });
    ...
    world.afterEvents.playerInventoryItemChange?.subscribe((e) => {
      try {
        ...
        console.error("[Jobs] Inventory change error:", error);
      }
    });
    ...
    world.afterEvents.playerDimensionChange?.subscribe((e) => {
      try {
        ...
        console.error("[Jobs] Dimension change error:", error);
      }
    });
    ...
    world.beforeEvents.chatSend?.subscribe((e) => {
      try {
        ...
        console.error("[Jobs] Chat command error:", error);
      }
    });
  }
  ...
  setContext(
    ...
    kind,
    extra = {},
    duration = CONFIG.INTERACTION_CONTEXT_TICKS,
  ) {
  ...
  handleBlockBreak(e) {
    ...
    system.runTimeout(
      () => this.recentBrokenLocations.delete(blockKey),
      CONFIG.BUILDER_LOOP_COOLDOWN_TICKS,
    );
    ...
    if (data.jobs.has("miner")) {
      ...
      if (reward) {
        const rewardResult = this.data.addReward(
          player,
          "miner",
          reward,
          blockType,
        );
        const rewardToast = formatRewardToast(
          rewardResult,
          t(player, "reward.mining"),
        );
        if (rewardResult.points > 0)
          this.sendRewardActionBar(player, rewardToast, 0.3);
  ...
  handleBlockPlace(e) {
    ...
    const isLoopPlacement =
      (recentlyBroken && system.currentTick - recentlyBroken <= loopWindow) ||
      (recentlyRewarded && system.currentTick - recentlyRewarded <= loopWindow);
    ...
    if (!isLoopPlacement) {
      ...
      if (reward && isBuildingBlock(blockType)) {
        ...
        const rewardToast = formatRewardToast(
          rewardResult,
          t(player, "reward.building"),
        );
        if (rewardResult.points > 0)
          this.sendRewardActionBar(player, rewardToast, 0.2);
        this.builderRewardLocations.set(key, system.currentTick);
        system.runTimeout(
          () => this.builderRewardLocations.delete(key),
          CONFIG.BUILDER_LOOP_COOLDOWN_TICKS,
        );
      }
    }
  ...
  handleEntityDeath(e) {
    ...
    if (data.jobs.has("hunter")) {
      ...
      if (reward) {
        ...
        const rewardToast = formatRewardToast(
          rewardResult,
          t(killer, "reward.hunting"),
        );
        if (rewardResult.points > 0) {
          if (
            isBoss &&
            rewardToast &&
            this.data.shouldNotify(killer, "rewardChat")
          )
            killer.sendMessage(`§c§l${rewardToast} §r§7(Boss Kill!)`);
          else this.sendRewardActionBar(killer, rewardToast, 0.3);
        }
        this.data.updateQuestProgress(
          killer,
          "hunter",
          isBoss ? "boss" : "mob",
          entityType,
  ...
  handleItemUse(e) {
    ...
    if (itemType === "minecraft:fishing_rod") {
      this.fishingPlayers.set(
        player.id,
        system.currentTick + CONFIG.FISHING_WINDOW_TICKS,
      );
    }
  }
  ...
  handlePlayerInteractWithEntity(e) {
    ...
    if (isTraderEntity(target.typeId)) {
      this.setContext(
        ...
        CONFIG.INTERACTION_CONTEXT_TICKS * 2,
      );
    }
  }
  ...
  handleInventoryChange(e) {
    ...
    for (const change of changes) {
      ...
      const effectiveGain = this.consumeInventoryDebit(
        player.id,
        change.typeId,
        change.delta,
      );
      ...
      if (data.jobs.has("fisherman") && fishingExpiry >= system.currentTick) {
        ...
        if (reward) {
          ...
          ].includes(change.typeId);
          const rewardResult = this.data.addReward(
            player,
            "fisherman",
            reward * effectiveGain,
            change.typeId,
          );
          const rewardToast = formatRewardToast(
            rewardResult,
            isTreasure
              ? t(player, "reward.treasure")
              : t(player, "reward.fishing"),
          );
          if (rewardResult.points > 0) this.sendRewardChat(player, rewardToast);
          this.data.updateQuestProgress(
            player,
            "fisherman",
            isTreasure ? "treasure" : "fish",
            change.typeId,
            effectiveGain,
          );
      ...
      if (context.kind === "crafting" && data.jobs.has("crafter")) {
        ...
        if (reward) {
          ...
          this.data.updateQuestProgress(
            ...
            "crafter",
            "craft",
            getCraftQuestCategory(change.typeId),
            effectiveGain,
          );
        }
        continue;
      ...
      if (context.kind === "blacksmith" && data.jobs.has("blacksmith")) {
        ...
        if (reward > 0) {
          ...
          this.data.updateQuestProgress(
            ...
            "blacksmith",
            "craft",
            getBlacksmithQuestCategory(change.typeId),
            effectiveGain,
          );
        }
        continue;
      ...
      if (context.kind === "alchemy" && data.jobs.has("alchemist")) {
        ...
        if (reward > 0) {
          ...
          this.data.updateQuestProgress(
            ...
            "alchemist",
            "craft",
            getAlchemyQuestCategory(change.typeId),
            effectiveGain,
          );
        }
        continue;
    ...
    if (
      ...
      changes.some(
        (change) => change.typeId === "minecraft:emerald" && change.delta < 0,
      )
    ) {
      this.scheduleDebitResolution(
        player,
        "minecraft:emerald",
        "trading",
        (spent) => {
          ...
          const reward =
            REWARDS.TRADING[rewardKey] || REWARDS.TRADING.common_trade;
          const rewardResult = this.data.addReward(
            player,
            "merchant",
            reward * spent,
          ...
          const rewardToast = formatRewardToast(
            rewardResult,
            t(player, "reward.trading"),
          );
          if (rewardResult.points > 0)
            this.sendRewardActionBar(player, rewardToast, 0.25);
    ...
    if (
      ...
      changes.some(
        (change) =>
          change.typeId === "minecraft:lapis_lazuli" && change.delta < 0,
      )
    ) {
      this.scheduleDebitResolution(
        player,
        ...
        (spent, ctx) => {
          const rewardResult = this.data.addReward(
            ...
            "enchanter",
            25 * spent,
            ctx.blockType?.replace("minecraft:", "") || "enchanting_table",
          );
          const rewardToast = formatRewardToast(
            rewardResult,
            t(player, "reward.enchanting"),
          );
          if (rewardResult.points > 0) this.sendRewardChat(player, rewardToast);
        },
      );
  ...
  handlePlayerSpawn(player) {
    system.runTimeout(() => {
      ...
      if (!hasPlayerLang(player))
        player.onScreenDisplay.setActionBar(
          t(player, "system.use_clock", { command: MENU_COMMAND }),
        );
  ...
  handleChatCommand(e) {
    ...
    if (cmd === "!jbal" || cmd === "!balance" || cmd === "!money") {
      e.cancel = true;
      player.sendMessage(
        t(player, "money.balance", {
          amount: formatNumber(this.getPlayerScore(player)),
        }),
      );
      return;
    }
    ...
    if (cmd === "!jhelp") {
      ...
      player.sendMessage(
        `§6--- Jobs Commands ---§r\n` +
          `§7!jbal §8- Check balance\n` +
          `§7!jpay <player> <amount> §8- Send money\n` +
          `§7!jhelp §8- Show this help\n` +
          `§7/jobs:gui §8- Open the jobs menu`,
      );
      if (player.hasTag(CONFIG.ADMIN_TAG)) {
        player.sendMessage(
          `§cAdmin: !jadmin give <player> <amount>\n` +
            `§cAdmin: !jadmin take <player> <amount>\n` +
            `§cAdmin: !jadmin reset <player>\n` +
            `§cAdmin: !jadmin reload`,
        );
      }
    }
  }
  ...
  handlePayCommand(player, args) {
    ...
    if (!target) {
      player.sendMessage(
        t(player, "money.pay.not_found", { player: targetName }),
      );
      return;
    }
    ...
    if (balance < amount) {
      player.sendMessage(
        t(player, "money.pay.insufficient", { balance: formatNumber(balance) }),
      );
      return;
    }

    this.addPlayerScore(player, -amount);
    this.addPlayerScore(target, amount);
    player.sendMessage(
      t(player, "money.pay.sent", {
        player: targetName,
        amount: formatNumber(amount),
      }),
    );
    target.sendMessage(
      t(target, "money.pay.received", {
        amount: formatNumber(amount),
        player: player.name,
      }),
    );
  ...
  handleAdminCommand(player, args) {
    ...
    switch (subCmd) {
      case "give": {
        if (args.length !== 4) {
          player.sendMessage("§cUsage: !jadmin give <player> <amount>");
          return;
        }
        const giveTarget = world
          .getAllPlayers()
        ...
        this.addPlayerScore(giveTarget, giveAmount);
        player.sendMessage(`§aGave ${args[2]} $${formatNumber(giveAmount)}`);
        giveTarget.sendMessage(`§aAdmin gave you $${formatNumber(giveAmount)}`);
        break;
      }
      case "take": {
        if (args.length !== 4) {
          player.sendMessage("§cUsage: !jadmin take <player> <amount>");
          return;
        }
        const takeTarget = world
          .getAllPlayers()
          .find((p) => p.name === args[2]);
        if (!takeTarget) {
          player.sendMessage("§cPlayer not found");
        ...
        const takeAmount = parseInt(args[3]);
        if (isNaN(takeAmount)) {
          player.sendMessage(t(player, "money.pay.invalid"));
          return;
        }
        this.addPlayerScore(takeTarget, -takeAmount);
        player.sendMessage(
          `§aTook $${formatNumber(takeAmount)} from ${args[2]}`,
        );
        break;
      }
      case "reset": {
        if (args.length !== 3) {
          player.sendMessage("§cUsage: !jadmin reset <player>");
          return;
        }
        const resetTarget = world
          .getAllPlayers()
          .find((p) => p.name === args[2]);
        if (!resetTarget) {
          player.sendMessage("§cPlayer not found");
        ...
        this.data.resetPlayerData(resetTarget);
        player.sendMessage(`§aReset all job data for ${args[2]}`);
        resetTarget.sendMessage("§cYour job data has been reset by an admin");
        break;
      }
      case "reload":
  ...
  getPlayerScore(player) {
    try {
      const obj = world.scoreboard.getObjective(CONFIG.OBJECTIVE_ID);
  ...
  addPlayerScore(player, amount) {
    try {
      ...
      if (obj && player.scoreboardIdentity) {
        obj.setScore(
          player.scoreboardIdentity,
          (obj.getScore(player.scoreboardIdentity) || 0) + amount,
        );
      }
    } catch (error) {
      console.error(error);
...
class SalarySystem {
  ...
  start() {
    if (!CONFIG.ENABLE_DAILY_SALARY) return;
      ...
      for (const player of players) {
        try {
          ...
          for (const jobId of data.jobs) {
            ...
            totalSalary += salary;
            breakdown.push(
              `${job.color}${jobNamePlain(player, job.id)} §a+$${formatNumber(salary)}`,
            );
          }

          if (totalSalary > 0) {
            const payoutResult = applyConfiguredRewardPayouts(
              player,
              "salary",
              {
            ...
            if (this.data.shouldNotify(player, "salaryMessages")) {
              ...
              player.sendMessage(`§6§lSALARY PAYMENT §r§7(Day ${dayCount})`);
              ...
              player.sendMessage(
                `§6Total Value: §a$${formatNumber(totalSalary)}`,
              );
              player.sendMessage("§6----------------------------");
            }
...
function registerJobsGuiCommand(customCommandRegistry) {
  try {
    customCommandRegistry.registerCommand(
      {
        name: JOBS_ADDON_CONFIG.menu.command.name,
        description: JOBS_ADDON_CONFIG.menu.command.description,
        permissionLevel:
          COMMAND_PERMISSION_LEVELS[
            JOBS_ADDON_CONFIG.menu.command.permissionLevel
          ] || CommandPermissionLevel.Any,
        cheatsRequired: JOBS_ADDON_CONFIG.menu.command.cheatsRequired ?? false,
      },
      (origin) => {
        ...
        return {
          ...
          message: "Opening Jobs menu...",
        };
      },
    );
  } catch (error) {
    console.error("[Jobs] Failed to register /jobs:gui command:", error);
  }
}

system.beforeEvents.startup.subscribe(({ customCommandRegistry }) => {
  registerJobsGuiCommand(customCommandRegistry);
});
...
// Initialize system
const jobsSystem = new JobsSystem();

// Export for external use
export { jobsSystem, JOBS, CONFIG, REWARDS, QUESTS, ACHIEVEMENTS };
